...
OUTPUT_DIR = "./output_predictions/segmentation_masks2"
...
for image_name in os.listdir(IMAGE_DIR):
    ...
    outputs_torn = predictor_torn(image_rgb)
    outputs_wrinkle = predictor_wrinkle(image_rgb)

    # Create separate visualizers (RGB input)
    v_torn = Visualizer(image_rgb, MetadataCatalog.get(DATASET_NAME_TORN), scale=1.0)
    v_torn = v_torn.draw_instance_predictions(outputs_torn["instances"].to("cpu"))

    v_wrinkle = Visualizer(image_rgb, MetadataCatalog.get(DATASET_NAME_WRINKLE), scale=1.0)
    v_wrinkle = v_wrinkle.draw_instance_predictions(outputs_wrinkle["instances"].to("cpu"))

    # Convert drawn images back to numpy arrays (still in RGB)
    mask_torn_rgb = v_torn.get_image()
    mask_wrinkle_rgb = v_wrinkle.get_image()

    # Convert images back to BGR for OpenCV
    mask_torn_bgr = cv2.cvtColor(mask_torn_rgb, cv2.COLOR_RGB2BGR)
    mask_wrinkle_bgr = cv2.cvtColor(mask_wrinkle_rgb, cv2.COLOR_RGB2BGR)

    # Blend the two images using weighted sum (to ensure visibility of both)
    blended_image = cv2.addWeighted(mask_torn_bgr, 0.5, mask_wrinkle_bgr, 0.5, 0)

    # Save the final blended image (in BGR format)
...
print("Inference complete. Segmented images saved in './output_predictions/segmentation_masks2/'")
